...
float Res_Value = 7.5;  // Resistor Value in Ohm
float Vcc = 4.69; // Voltage of Arduino 5V pin ( Mesured by Multimeter )
...
float Bat_Low = 2.8; // Discharge Cut Off Voltage
...
void draw(void) {
  ...
  */
   ...
   Serial.print("Volt: "); 
   Serial.print( Bat_Volt,2);  // display Battery Voltage in Volt
   Serial.println("V"); 
...
//*******************************Setup Function ***************************************************************
  ...
  void setup() {
   ...
   digitalWrite(MOSFET_Pin, HIGH);  // MOSFET is off during the start
   delay(5000);
 ...
 // Vcc = readVcc()/1000.0; // Conevrrt mV to Volt
  ...
  if ( Bat_Volt > Bat_High){
    digitalWrite(MOSFET_Pin, HIGH); // Turned Off the MOSFET // No discharge 
    beep(200);
